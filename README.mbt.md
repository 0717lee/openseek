...
| `bobzhang/openseek/logger` | Native-only async JSONL writer. | `logger/README.mbt.md` |
...
the CLI to override it for one run. Logs are JSONL by default so they can be
processed with `jq` while the agent is still running.
