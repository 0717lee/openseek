...
`bobzhang/openseek/agent_tool/edit/internal/manifest_error` keeps generated-file
and manifest safety checks used before the `edit` tool writes replacement
content.
...
error message for generated files and manifest rewrites that look like common
agent mistakes.

## API Shape

- `path_error(path)`: returns `Some(message)` when the target path should be
  rejected regardless of content, otherwise `None`.
- `write_error(path, content)`: returns `Some(message)` when the proposed write
  should be rejected, otherwise `None`.

## Checks

The guard is intentionally narrow and only handles generated MoonBit interface
files and MoonBit manifest paths:

- `pkg.generated.mbti` files are rejected because they are generated by
  `moon info`.
