...
static int async_trace_fd = -1;
static uint64_t async_trace_sequence = 0;
static pthread_mutex_t async_trace_mutex = PTHREAD_MUTEX_INITIALIZER;

static
void async_trace(const char *event, const char *format, ...) {
  ...
  pthread_mutex_lock(&async_trace_mutex);
  uint64_t sequence = async_trace_sequence++;
  char line[768];
  int length = snprintf(
    line,
    sizeof(line),
    "%lld.%06d seq=%llu pid=%d thread=0x%llx event=%s %s\n",
  ...
  int offset = 0;
  int fd = async_trace_fd;
  if (fd < 0) {
    pthread_mutex_unlock(&async_trace_mutex);
    errno = saved_errno;
    return;
  }
  while (offset < length) {
    int written = write(fd, line + offset, length - offset);
    if (written > 0) {
      offset += written;
...
static
void async_trace_initialize() {
  const char *configuration = getenv("MOONBIT_ASYNC_TRACE");
  if (
    configuration == 0 ||
    configuration[0] == '\0' ||
    strcmp(configuration, "0") == 0
  )
    return;

  char default_path[128];
  const char *path = configuration;
  if (strcmp(configuration, "1") == 0) {
    snprintf(
      default_path,
      sizeof(default_path),
      "/tmp/moonbit-async-%d.log",
      (int)getpid()
    );
    path = default_path;
  }
  ...
  async_trace_fd = fd;
  async_trace("trace_start", "path=%s", path);
}

static
void async_trace_shutdown() {
  if (async_trace_fd < 0)
    return;

  async_trace("trace_stop", "notify_send=%d", pool.notify_send);
  pthread_mutex_lock(&async_trace_mutex);
  int fd = async_trace_fd;
  async_trace_fd = -1;
  pthread_mutex_unlock(&async_trace_mutex);
...
static
void notify_completion(const char *source, int32_t value) {
  int result;
  int write_errno;
  do {
    result = write(pool.notify_send, &value, sizeof(value));
    write_errno = errno;
    if (result > 0)
      break;
  } while (write_errno == EINTR);

  async_trace(
    "completion_write",
    "source=%s notify_send=%d raw=0x%08x signed=%d bytes=%d errno=%d",
    source,
    pool.notify_send,
    (unsigned int)value,
    value,
    result,
    result < 0 ? write_errno : 0
  );
}
...
#else
    notify_completion("worker", job_id);
#endif
...
void moonbitlang_async_init_thread_pool(HANDLE notify_send) {
  ...
  async_trace(
    "pool_init",
    "notify_send=%d wakeup_method=condition_variable",
    pool.notify_send
  );
#else
  async_trace("pool_init", "notify_send=%d wakeup_method=signal", pool.notify_send);
#endif
#endif
}
...
#ifndef _WIN32
  async_trace("pool_destroy", "notify_send=%d", pool.notify_send);
  pthread_sigmask(SIG_SETMASK, &pool.old_sigmask, 0);
  async_trace_shutdown();
#endif
}
...
int32_t moonbitlang_async_fetch_completion(int notify_recv, int32_t *output) {
  ...
  int result = read(notify_recv, (char*)output, max_len * sizeof(int32_t));
  int read_errno = errno;
  async_trace(
    "completion_read",
    "notify_recv=%d bytes=%d errno=%d capacity=%d",
    notify_recv,
    result,
    result < 0 ? read_errno : 0,
    max_len
  );
  if (result > 0) {
    int count = result / sizeof(int32_t);
    for (int i = 0; i < count; ++i) {
      uint32_t raw = (uint32_t)output[i];
      if ((raw & 0x80000000u) != 0) {
        async_trace(
          "completion_read_high_bit",
          "notify_recv=%d index=%d raw=0x%08x decoded_signal=%u",
          notify_recv,
          i,
          raw,
          raw & 0x7fffffffu
    ...
    if ((result % sizeof(int32_t)) != 0) {
      async_trace(
        "completion_read_partial",
        "notify_recv=%d bytes=%d remainder=%d",
        notify_recv,
        result,
        result % (int)sizeof(int32_t)
      );
    }
  }
  errno = read_errno;
...
void sigwait_job_worker(struct job *job) {
  ...
  while (1) {
    ...
    if (err == 0) {
      ...
      async_trace(
        "sigwait_return",
        "result=%d signal=unavailable errno=%d",
        err,
        sigwait_errno
      );
    }
    ...
    if (sig == SIGUSR2) {
      async_trace("sigwait_stop", "signal=%d", sig);
      break;
    }
