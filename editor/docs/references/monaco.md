...
parity evidence lives in source citations and focused reference tests against
the checked-in `vscode` submodule.
...
contract and its 23-case conformance denominator use the checked-in `vscode`
submodule.
...
local `internal/viewer/browser/view` package. The current source mapping is:
