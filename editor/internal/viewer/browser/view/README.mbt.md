...
```mermaid
flowchart TB
  VM["ViewModel + ViewLayout<br>(DOM-free)"] --> V["View — one package, one compilation unit"]
  V --> P1["virtualized lines + recycler"]
  V --> P2["margin / line numbers / decorations"]
  V --> P3["selections + cursor + current line"]
  V --> P4["content + overlay widgets, zones"]
  V --> P5["scrollbar view part"]
  P1 --> DOM["DOM"]
  P2 --> DOM
  P3 --> DOM
  P4 --> DOM
...
Its focused `.mbt` files preserve source-unit responsibilities; they are not
package or namespace boundaries. Everything in this directory shares private
declarations as one compilation unit, which is what lets the render/event
ordering stay internal.

```mbt nocheck
// The root Viewer owns construction and ordering; nothing outside this package
// builds a View. Sketch of the ownership the root holds:
let view = View::new(view_model, layout, theme, scheduling)
view.render(rendering_context)   // one ordered pass: prepare, then write DOM
view.dispose()                   // releases DOM, listeners, and the recycler
```
...
These are the required checks documented in `docs/quality.md`; run the subset
relevant to a milestone and all of them before declaring cross-package or
browser-visible work complete.
