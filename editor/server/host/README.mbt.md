...
Host-effect adapter and executable backend for the reference shell. The package
supports Wasm and native; the server module and `justfile` select Wasm by
default, with `TARGET=native` as an explicit development override.
...
package accepts `--root`, `--host`, `--port`, `--asset-dir`, `--moon-command`,
and `--browse-root`.
...
`ROOT` selects the initial workspace, while host browsing and workspace
switching have a separate safety scope:

- with an empty browse root and a loopback listener (`localhost`, `::1`, or a
  `127.*` address), the repository picker may browse the whole host filesystem;
- with an empty browse root and any other listener, repository browsing and
  switching are disabled; and
- an explicit `--browse-root <directory>` wins on either bind and permits only
  that realpathed directory and its descendants. The picker omits the parent at
  the scope boundary, and out-of-scope paths are rejected before their
  existence is probed.

The Just equivalent is `BROWSE_ROOT=<directory>`. If the initially served
`ROOT` is outside that explicit subtree, it remains the initial workspace but
the picker starts at `BROWSE_ROOT`.
...
This package supports the server module's Wasm and native targets and is not
part of the reusable viewer API. It owns concrete host/provider behavior, but
not protocol packet routing or browser UI.
