...
// The workspace switch is server-process-wide state, so this test always
// restores the original fixture root before finishing.
test('browses the host filesystem and switches repositories', async ({ page }) => {
  let restoreFixtureWorkspace = false;
  try {
    await page.goto('/');
    await expect(
