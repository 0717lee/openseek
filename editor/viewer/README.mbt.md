...
model range. Cursor, selection, ViewZones, editor mouse events, Code folding,
quick diff, feedback widgets, Markdown comments, and Code-view overlays remain
...
Markdown section folding is a separate root-Viewer path. The markdown package
derives sections, reconciliation keys, and default collapses; the browser view
owns only fold controls and retained-node visibility. A new model seeds the
default policy, a theme replacement preserves state, and a source replacement
retains a reader decision only for an unambiguous section whose body is
unchanged. Changed, new, or ambiguous sections expand, and a model swap resets
the state. Standalone toggles publish final visibility before invalidating the
hover bridge; replacement reconciles visibility inside the existing projection
replacement bracket.
...
- Markdown section folding through `toggle_markdown_section_at` and
  `collapsed_markdown_sections`;
