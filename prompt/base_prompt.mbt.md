...
- Get each batch right the first time — that is the goal, not applying fast and fixing later. Derive one edit per diagnostic with a precise, receiver-qualified `old_string`, and check that the edit count is close to the diagnostic count before applying (far more edits than diagnostics means you over-matched: fix the generation, do not apply it). `edit_undo` exists only as a last-resort safety net, not a routine step: if a batch you already applied *unexpectedly* breaks the build (run `moon check` to confirm), call `edit_undo` to revert that edition and regenerate precisely, rather than hand-untangling a broken tree. Do not apply speculative or over-broad batches expecting to undo them — undoing and redoing burns steps and rarely beats getting it right once. `edit_undo` walks back the most recent edit/multi_edit/write editions and refuses if a file changed since, so it never discards newer work.
