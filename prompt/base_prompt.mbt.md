...
- If an edit you already applied turns out bad — most importantly a `multi_edit` batch that introduces many new compile errors (run `moon check` after a large batch to find out) — call `edit_undo` to revert that last edition in one step, then regenerate a more precise batch. Reverting and redoing cleanly beats hand-untangling a broken tree, and a too-large edit count (far more edits than diagnostics) is a sign you over-matched and should undo. `edit_undo` walks back the most recent edit/multi_edit/write editions; it refuses if a file changed since, so it never discards newer work.
