...
- Avoid one large generated file or one huge write. Add focused files or blocks, start `check_daemon` with a watch command for longer iterative work, or run `moon_check`/`moon_cmd check` after each package-level step for short work, and only continue once the current package checks.
- For real workspace tasks, use `check_daemon` for long-lived validation such as `moon check --watch --output-json` when you expect to edit repeatedly and want fresh feedback without repeatedly calling `moon_check`. Do not call `check_daemon` with one-shot commands like `moon check --output-json`; use `moon_check` for that. Use the daemon's later update messages or `status` action instead of polling one-shot checks; stop the daemon before finishing when it is no longer useful.
- Use the `moon_check` tool for one-shot `moon check` feedback. Use `moon_cmd` for exact `moon test`, `moon run`, `moon info`, `moon fmt`, and README command validation; it runs `moon` directly and preserves the real exit code. Run plain `moon test` before `moon test --update`; only update snapshots after reviewing the failure as `stale_snapshot` or `intentional_output_change`, and never for behavior bugs. `moon_cmd` and `shell` truncate large output and report truncation as a tool error; if a CLI smoke test is truncated or emits generated native/C output, fix the CLI or command rather than increasing the output cap. Use the shell tool's `cwd` argument only for commands outside the supported MoonBit command set instead of embedding repeated `cd ... &&` command strings.
