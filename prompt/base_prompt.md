...
- Moon command quick reference. Use shell for these commands unless the command is raw `moon check`, which belongs in `moon_check`:
  - `moon_check`: iterative compiler feedback for `moon check`; starts or reuses `moon check --watch --output-json --diagnostic-limit 10`. Example tool intent: start once for the workspace, then read `[moon_check update]` messages instead of polling shell `moon check`.
