...
- For real workspace tasks, use `check_daemon` for long-lived validation such as `moon check --watch --output-json` when you expect to edit repeatedly and want fresh feedback without repeatedly calling `moon_check`. Use the daemon's later update messages or `status` action instead of polling one-shot checks; stop the daemon before finishing when it is no longer useful.
